...
`clusterCR3Score`, rewrites the CR3 sandwich through that score API, and includes the deterministic
conservativeness bridge `olsClusteredCR3VarianceEstimator_conservative_of_middle`: middle-matrix PSD
dominance implies sandwich-level PSD dominance. The multi-observation cluster block layer now also has the
...
conservativeness middle-moment comparison; otherwise continue to the remaining Chapter 6 and Chapter 7
asymptotic gaps.
...
- The remaining covariance-estimator gaps are clustered asymptotics beyond these finite block
  identities and the CR3 middle-moment comparison needed to discharge full conditional
  conservativeness.

...
covariance estimators reduce to the heteroskedastic sandwich estimators.  The
cluster score middle matrix and the unadjusted clustered sandwich are proved
positive semidefinite via \lean{sum_vecMulVec_posSemidef},
\lean{olsClusteredScoreMiddle_posSemidef}, and
\lean{olsClusteredVarianceEstimator_posSemidef}; the adjusted sandwich is PSD
whenever its finite-sample scale factor is nonnegative, and
\lean{clusterFiniteSampleAdjustment_nonneg} discharges that condition from
$k<n$ and $G>1$.  The CR3-style leave-cluster-out layer now has
\lean{ClusterIndex}, \lean{clusterDesign}, \lean{clusterCR3Residual},
\lean{clusterLeaveOutBeta_eq_olsBeta_sub_invGram_mulVec_cr3Residual},
\lean{clusterCR3Score}, \lean{clusterCR3ScoreMiddle},
\lean{olsClusteredCR3VarianceEstimator},
\lean{olsClusteredCR3VarianceEstimator_eq_clusterCR3Score},
\lean{olsClusteredCR3VarianceEstimator_eq_clusterCR3ScoreMiddle},
\lean{olsClusteredCR3VarianceEstimator_conservative_of_middle},
\lean{condExp_deterministic_sandwich_eq},
\lean{condExp_olsClusteredCR3VarianceEstimator_eq_sandwich_of_middle},
\lean{condExp_olsClusteredCR3VarianceEstimator_conservative_of_middle},
\lean{condExp_olsClusteredCR3VarianceEstimator_conservative_of_block_middle}, and
\lean{olsClusteredCR3VarianceEstimator_posSemidef}, covering the deterministic
equations (4.52), (4.53), and (4.54) at the estimator-definition/named-score/PSD
level and reducing deterministic and conditional CR3 conservativeness to
middle-matrix PSD dominance.  The block monotonicity lemmas
\lean{clusterCovarianceMiddle_mono_posSemidef} and
\lean{olsClusterConditionalVarianceMatrix_mono_posSemidef} reduce that dominance
to clusterwise block dominance.  The CR3 linear-model rewrites
\lean{clusterResidual_linear_model}, \lean{clusterCR3Residual_linear_model},
\lean{clusterCR3Score_linear_model}, and
\lean{clusterCR3ScoreMiddle_linear_model} reduce the remaining CR3 block moments
to deterministic transforms of annihilator-transformed structural errors.  This
tightens the finite-sample Gauss--Markov and covariance-estimator proof layer.
The cluster-block layer now also proves the finite partition identity
\lean{clusterIndex_sum_eq_sum}, the Gram and cross-product decompositions
\lean{sum_clusterGramContribution_eq_gram} and
\lean{sum_clusterCrossContribution_eq_cross}, the named-score sandwich rewrite
\lean{olsClusteredVarianceEstimator_eq_clusterScore}, and the residual
cluster-score normal-equation consequence \lean{sum_clusterScore_eq_zero}.
It also names Hansen's clustered covariance middle and sandwich formulas through
\lean{clusterCovarianceMiddle} and
\lean{olsClusterConditionalVarianceMatrix}, proves PSD inheritance from PSD
cluster covariance blocks, and records the block outer-product/score-middle
bridges \lean{clusterCovarianceMiddle_outer_eq_scoreMiddle},
\lean{clusterErrorScore},
\lean{olsBeta_linear_decomposition_clusterScores},
\lean{clusterCovarianceMiddle_errorOuter_eq_clusterErrorScoreMiddle},
\lean{condExp_clusterErrorScoreMiddle_entry_eq_clusterCovarianceMiddle},
\lean{condExp_clusterErrorScoreMiddle_eq_clusterCovarianceMiddle},
\lean{condExp_clusterErrorScoreSandwich_eq_olsClusterConditionalVarianceMatrix},
\lean{olsClusteredVarianceEstimator_eq_clusterCovarianceMiddle_residualOuter},
and \lean{olsClusteredCR3VarianceEstimator_eq_clusterCovarianceMiddle_cr3Outer}.

\section{Chapter 5: Normal-Regression Moment Face}

The exact random-design Kinal threshold is still not proved.  The branch does
now record the fixed-design finite-moment consequence of the Gaussian
coefficient law: \lean{olsBeta_memLp_of_error_gaussian} shows that, once $X$ is
fixed and nonsingular and the error vector is Gaussian, the OLS coefficient
vector belongs to every finite $L^p$ space.  This isolates the remaining Kinal
work as the inverse random-Gram tail threshold rather than the Gaussian
finite-moment part.
...
endpoints.  The scalar Lyapunov-to-Lindeberg algebra now includes
\lean{sq_le_even_moment_div_threshold} and
\lean{sq_tail_indicator_le_even_moment_div_threshold}, which bound the scalar
Lindeberg tail summand by an even higher moment over the threshold power, plus
\lean{integral_sq_tail_le_integral_even_moment_div_threshold}, which lifts that
bound to expectations.  The finite-row and normalized forms are
\lean{sum_integral_sq_tail_le_sum_integral_even_moment_div_threshold} and
\lean{normalized_sum_integral_sq_tail_le_normalized_even_moment_bound}.  The
remaining scalar triangular-array probability engine remains outside this
branch.
...
\begin{itemize}
  ...
  \item \lean{variance_le_of_eq_add_uncorrelated}.
  \item \lean{covMat_sub_posSemidef_of_eq_add_uncorrelated}.
  \item \lean{iidEstimator_variance_ge_sampleMean_of_mean_add_uncorrelated}.
  \item \lean{iidEstimator_covMat_sub_sampleMean_posSemidef_of_mean_add_uncorrelated}.
...
unbiased estimator improves on the equal-weight sample mean.  The orthogonal
residual lemmas also isolate the Hilbert-space variance algebra and compose it
with the sample mean for estimators of the form sample mean plus an uncorrelated
residual.  They are not the full arbitrary-unbiased-estimator lower bound of
Hansen Theorem 6.11 because the theorem-specific orthogonality discharge remains
missing.
...
these wrappers from its fourth-row-moment assumption. The branch also adds
\lean{IidAssumption72FourthMomentConditions}, which derives the squared-error
moment and true-error score outer-product moment from structural-error and
regressor fourth moments, then converts to
\lean{IidRobustFeasibleHCMomentConditions}. It further adds
\lean{IidAssumption72ResponseMomentConditions}, which derives the structural-error
fourth moment from the response fourth moment, the linear-model identity, and the
regressor fourth-row moment before reusing the fourth-moment package. The
combined robust feasible-HC
package and the iid package now also derive the feasible-HC third absolute
row-weight moment $\mathbb{E}\{|e_0|\|X_0\|^3\}<\infty$ from the score
outer-product moment and the fourth-row moment, rather than carrying it as a
separate primitive field in those public packages.

\subsection{Edgeworth Interface}

The branch does not prove Hansen's concrete high-moment cumulant expansion.
It does add generic interfaces and the polynomial-shape layer that future work
can instantiate:
\begin{itemize}
  ...
  \item \lean{edgeworthP1Polynomial} and \lean{edgeworthP2Polynomial}, recording
    the even quadratic and odd degree-five textbook shapes;
  \item \lean{SecondOrderEdgeworthExpansion.symmetric_interval_scaled_remainder_tendsto_zero},
    proving the two-sided cancellation of the $n^{-1/2}$ Edgeworth term.
  \item \lean{SecondOrderEdgeworthExpansion.symmetric_interval_scaled_remainder_tendsto_zero_polynomial},
    the coefficient-specialized version for the Hansen polynomial forms.
\end{itemize}

Thus Theorem 7.15 has a reusable formal target shape and the central-interval
cancellation consequence, but not the high-moment cumulant coefficient proof.
...
Area & Remaining work \\
\midrule
Chapter 4 & Clustered covariance asymptotics beyond the finite cluster-block
identities; raw CR3 middle-moment identification needed to construct conditional
covariance matrices for adjusted annihilator-error blocks from primitive cluster
error assumptions. Hansen's Section 4.23 CR3 conservativeness sentence appears
to be a finite-sample conditional-on-\(X\) claim, not an asymptotic claim; the
remaining proof should identify the leave-cluster conditional moment
\(E[\tilde e_g \tilde e_g' \mid X]\) and feed the resulting blockwise positive
semidefinite dominance into the existing CR3 sandwich bridge. \\
...
probability-engine discharge for Theorems 6.4 and 6.5 beyond the deterministic
and expected finite-row even-moment tail inequalities; arbitrary-unbiased-estimator
lower bound for Theorem 6.11. \\
Chapter 7 & Further tightening of the remaining score-CLT public package toward
literal textbook iid assumptions; concrete high-moment cumulant expansion for
Theorem 7.15. \\
\bottomrule
\end{longtable}

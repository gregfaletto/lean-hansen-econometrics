...
iid observations and that no scalar linear unbiased weighted estimator improves
on the equal-weight sample mean.  They are not the full
...
CLT package from the score outer-product moment. The iid package now also derives
the feasible-HC third absolute row-weight moment
$\mathbb{E}\{|e_0|\|X_0\|^3\}<\infty$ from the score outer-product moment and
the fourth-row moment, rather than carrying it as a separate primitive iid
field.
